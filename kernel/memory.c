...
//因为 0xc009f000 是内核主线程栈顶，0xc009e000 是内核主线程的 pcb。 在loader程序设置的esp指针地址决定了0xc009e000是内核PCB的起始地址
//PCB的地址必须是自然页0xXXXXX000到0xXXXXXfff 
//虚拟机配置的32m，32m需要1024个字节位图去作控制，但是只是占页的四分之一，一页可以管理128m内存，放肆一下，用4页来管理，控制512m内存
//故位图的起始地址就是这个
#define MEM_BITMAP_BASE 0xc009a000 

#define PDE_IDX(addr) ((addr & 0xffc00000) >> 22)   //用于返回虚拟地址的高地址
#define PTE_IDX(addr) ((addr & 0x003ff000) >> 12)   //用于返回虚拟地址的中地址
    ...
    ***********************************************************/ 
    ...
    if (*pde & 0x00000001) { 
        ...
        * 把低 12 位置 0 便是该 pde 对应的物理页的起始*/ 
        //因为pte是访问页表的虚拟地址，那这个地址去索引，能清空这个页表的数据
